...
namespace Winfocus.LMS.Application.Services
{
    ...
    public class BatchTimingSaturdayService : IBatchTimingSaturdayService
    {
        ...
        public async Task<CommonResponse<List<BatchTimingSaturdayDto>>> GetAllAsync(Guid centerId)
        {
            try
            {
        ...
        /// <param name="centerId">The centerId.</param>
        /// <returns>BatchTimingSaturdayDto.</returns>
        public async Task<CommonResponse<BatchTimingSaturdayDto>> GetByIdCenterIdAsync(Guid id, Guid centerId)
        {
            try
            {
                _logger.LogInformation("Fetching batchtiming by Id: {Id}", id);
        ...
        public async Task<CommonResponse<BatchTimingSaturdayDto>> UpdateAsync(Guid id, BatchTimingRequest request)
        {
            try
            {
                ...
                batch.BatchTime = DateTime.SpecifyKind(request.batchTime, DateTimeKind.Utc);
        ...
        {
            var batchtiming = new SubjectBatchTimingSaturday
            {
                SubjectId = request.subjectId,
                SubjectBatchTimingSaturdays = request.Batchtimingids
                    .Distinct()
                    .Select(id => new SubjectBatchTimingSaturday { SubjectId = id })
                    .ToList(),
            };
